    ...
    try {
      ...
      switch (acao) {
        ...
        case 'ATUALIZAR_PROGRESSO_FUNERAL':
          ...
          await client.query(upsertQuery, valoresUpsert);

          let protocoloAposUpdate = protocolo;
          if (dados.campos.status_sepultamento === 'realizado') {
            const updateRes = await client.query("UPDATE protocolos SET status_sepultamento = 'concluido' WHERE id = $1 RETURNING *;", [protocoloId]);
            protocoloAposUpdate = updateRes.rows[0];
          }
          
          if (protocoloAposUpdate.status_sepultamento === 'concluido' && protocoloAposUpdate.status_documentacao === 'concluido') {
            const finalRes = await client.query("UPDATE protocolos SET status = 'finalizado', data_finalizado = NOW() WHERE id = $1 RETURNING *;", [protocoloId]);
            protocoloAposUpdate = finalRes.rows[0];
          }
          
          await client.query('COMMIT');
          const progressoResult = await db.query('SELECT * FROM progresso_funeral WHERE protocolo_id = $1', [protocoloId]);
          return { progresso: progressoResult.rows[0], protocolo: protocoloAposUpdate };
        ...
        case 'ANEXAR_CERTIDAO_FINAL':
          ...
          if (protocolo.status_sepultamento === 'concluido') {
            queryUpdate = "UPDATE protocolos SET status_documentacao = 'concluido', status = 'finalizado', data_finalizado = NOW() WHERE id = $1 RETURNING *;";
            valoresUpdate = [protocoloId];
          } else {
            queryUpdate = "UPDATE protocolos SET status_documentacao = 'concluido' WHERE id = $1 RETURNING *;";
            valoresUpdate = [protocoloId];
          }
          break;
      ...
      console.log(`Protocolo ${protocoloId} transicionado. (Ação: '${acao}')`);
      return rows[0];
